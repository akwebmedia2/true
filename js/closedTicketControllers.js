 ...
 */
angular.module('TrueSense.controllers').controller('closedTicketCntrl', function($scope, $rootScope, Utils, database, $state, $localstorage, $timeout, applicationServices, $q, databasecache, pendingTicketUploadProcess, $window, $http) {

	$scope.$parent.$parent.$parent.app_page_title = closedTicketTitle;
	...
	$scope.ticketDetailsPage = function(details, type) {
		...
		$state.go('eventmenu.closedticketdetail');
	};

	/**
	 *Show ticket details page for offline
	 */
	$scope.logticketDetailsPage = function(details, type) {
		...
		database.getPendingTickets(ssoid, function(result) {
			if (result && result.rows && result.rows.length > 0) {
				for (var i = result.rows.length - 1; i >= 0; i--) {
					if (result.rows.item(i) && result.rows.item(i).Type && result.rows.item(i).Type == 'CLOSED_TICKET') {

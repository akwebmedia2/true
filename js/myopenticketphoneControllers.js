 ...
 */
angular.module('TrueSense.controllers').controller('myopenticketphoneCtrl', function($scope, Utils, database, $ionicScrollDelegate, applicationServices, $localstorage, $rootScope, $state, databasecache, pendingTicketUploadProcess) {
	$scope.$parent.$parent.$parent.app_page_title = myOpenTicketPageTitle;
				...
				database.getPendingTickets(ssoid, function(result) {
					if (result && result.rows && result.rows.length > 0) {
						...
						for (var i = result.rows.length - 1; i >= 0; i--) {
							...
							/*if (result.rows.item(i).UserId == ssoid && result.rows.item(i).Type != "ACCOUNTHIERARCHY" && result.rows.item(i).Type != 'MY_GROUP_EDIT' && result.rows.item(i).Type != 'MY_SPARE_PART' && result.rows.item(i).Type != 'CORRECTION' && result.rows.item(i).Type != 'NEWACCOUNT' && result.rows.item(i).Type != 'MY_GROUP_SPARE_PART' && result.rows.item(i).Type != 'CLOSED_TICKET') {
	...
	/**
	 *Create the ui only for my open tickets for mobile screen
	 * @param {Object} ssoid
	 */
			...
			database.getPendingTickets(ssoid, function(result) {
				...
				if (result && result.rows && result.rows.length > 0) {
					for (var i = result.rows.length - 1; i >= 0; i--) {
						...
						if (result.rows.item(i).UserId == ssoid && result.rows.item(i).Type == 'MY_SPARE_PART') { // For Spare part from ticket/log
							$scope.pendingSPItems.push(tktData);
						}

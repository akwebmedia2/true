 ...
 */
angular.module('TrueSense.controllers').controller('openticketCtrl', function($scope, $ionicScrollDelegate, Utils, $filter, $rootScope, $q, database, $parse, $http, $localstorage, $timeout, applicationServices, $state) {
	...
	$scope.cityserach = {};
	$scope.ticketform = {};
	$scope.ticketform.u_status = "Open";
	...
	$scope.uploadTicket = function() {
		//console.log($scope.ticketform.u_asset_number)
		$http({
			method : 'POST',
	...
	$scope.uploadTicketProcess = function(savedItems) {
		...
		if (ssoid) {
			...
			// Uploading Serial Correction Data
			if ($rootScope.isOnline()) {
				...
				database.storePendingTicket($scope.ticketform.ticketId, $scope.ticketform.userId, "NEW", $scope.ticketform.savedTS, $scope.ticketform, function(status) {
					$scope.ticketform = {};
					$scope.backToTicketsListing();
				});
	...
	$scope.ticketSubmit = function() {
		...
		if (savedItems) {
			...
			} else if (!(savedItems.u_issue_start_date && savedItems.u_issue_start_date.length > 0)) {
				Utils.showAlert(issueStartEmpty);
					...
					if (!(savedItems.u_defect_category2_2 && savedItems.u_defect_category2_2.length > 0)) {
						...
						if (!(savedItems.u_defect_category2_3 && savedItems.u_defect_category2_3.length > 0)) {
							...
							if (!(savedItems.u_defect_category2_4 && savedItems.u_defect_category2_4.length > 0)) {
								...
								} else if (savedItems.u_issue_end_date && savedItems.u_issue_end_date.length > 0) {
									try {
										//Check if start date must be less than end date
										var endTime = new Date(savedItems.u_issue_end_date).getTime();
										var startTime = new Date(savedItems.u_issue_start_date).getTime();
										if (startTime > endTime) {
											Utils.showAlert(issueEndDate);
										} else {
	...
	$scope.showAccountHierData = function() {
		...
		$scope.formData = [// Creating the static listing of form data
		{
			...
			"u_issue_start_date" : $scope.ticketform.u_issue_start_date,
		...
		//$localstorage.set('PARTIAL_DATA', 'YES');
		$localstorage.set('PARTIAL_FORM_DATA', JSON.stringify($scope.formData));
	...
	$scope.loadPartialData = function(formData) {
		...
		$scope.ticketform.u_issue_start_date = formData[0].u_issue_start_date;
	...
	var jumpFromAcHier = $localstorage.get('Partial_Account_Hieararchy_New');
	if (jumpFromAcHier) {
		var jumpFromAcHierData = angular.fromJson(jumpFromAcHier);
	}

	var showAccountHier = $localstorage.get('PARTIAL_FORM_DATA');
	if (showAccountHier) {
